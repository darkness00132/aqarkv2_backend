...
namespace Application.Services
{
    public class S3StorageService : IStorageService
    {
        ...
        public S3StorageService(IAmazonS3 S3, IOptionsSnapshot<S3Settings> options)
        {
            _s3 = S3;
        ...
        public async Task<List<string>> UploadManyAsync(IEnumerable<Stream> images, CancellationToken ct = default)
        {
            ...
            try
            {
                ...
                {
                    string KeyName = $"products/{Guid.NewGuid():N}.webp";
            ...
            {
                await this.DeleteManyAsync(PublicUrls, ct); // Rollback any uploaded images on failure
                throw new Exception("Failed to upload all images. Rolled back any uploaded images.");
            }
        }

        public async Task<string> UploadOneAsync(Stream image, CancellationToken ct = default, bool IsAvatar = false)
        {
            string KeyName = $"avatar/{Guid.NewGuid():N}.webp";
        ...
        private async Task<MemoryStream> ConvertToWebpAsync(Stream stream, bool IsAvatar = false)
        {
            ...
            {
                Size = IsAvatar ? new Size(256, 256) : new Size(1000, 1000),
